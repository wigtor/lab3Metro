<?php
	class Controller {
		...
		public static function getAllTrenes() {
			$connect = mysql_connect("localhost","metro", "metro123");
			$db = mysql_select_db("metroSantiago", $connect);
			
			$result = mysql_query("SELECT * FROM Tren NATURAL JOIN Estado_tren WHERE nombre_estado_tren = 'En cochera' OR nombre_estado_tren = 'En mantencion'");
			$matrix = NULL;
			$i = 0;
			while($fila = mysql_fetch_assoc($result)){
				$matrix[$i] = $fila;
